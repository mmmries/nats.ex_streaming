defmodule Nats.Streaming.ClientTest do
  ...
  describe "disconnected state" do
    ...
    test "failed connect attempts keep the state the same and retries connection" do
      ...
      assert {:keep_state_and_data, actions} =
               Client.disconnected(:internal, {:find_connection, nil}, state)
    ...
    test "finding the connection moves to connected status, tries to register client" do
      ...
      assert {:next_state, :connected, state, actions} =
               Client.disconnected(:internal, {:find_connection, self()}, state)
    ...
    test "it returns an error for pub info" do
      ...
      assert {:keep_state_and_data, actions} =
               Client.disconnected({:call, :from}, :pub_info, state)
  ...
  describe "connected state" do
    ...
    test "succesful connect_response messages move to registered state", %{state: state} do
      assert {:next_state, :registered, state, actions} =
               Client.connected(:internal, {:connect_response, @connect_response}, state)

      assert state.close_subject == @connect_response.closeRequests

      assert state.pub_subject ==
               "#{@connect_response.pubPrefix}.#{state.client_id}.#{state.conn_id}"
    ...
    test "failed connect_response does a delayed re-register", %{state: state} do
      ...
      assert {:keep_state_and_data, actions} =
               Client.connected(:internal, {:connect_response, response}, state)
    ...
    test "connection dying pushes back to disconnected status", %{state: state} do
      ...
      assert {:next_state, :disconnected, state, actions} =
               Client.connected(:info, down_message, state)
  ...
  describe "registered state" do
    setup do
      ...
      {:next_state, :registered, state, _actions} =
        Client.connected(:internal, {:connect_response, @connect_response}, state)
    ...
    test "connection dying pushes back to disconnected status", %{state: state} do
      ...
      assert {:next_state, :disconnected, state, actions} =
               Client.registered(:info, down_message, state)
